# Name
loadout-name-edit-label = Задает пользовательское имя, которое будет использоваться, если вы будете играть эту роль. Если пустое значение, то вместо него будет использоваться имя вашего персонажа.
loadout-name-edit-label-dataset = Задает пользовательское имя, которое будет использоваться, если вы будете играть за эту роль. Если оно пустое, вместо него будет выбрано случайное имя.
loadout-name-edit-tooltip = Не более { $max } символов. Если имя не указано, оно будет выбрано случайным образом.
# Restrictions
loadout-restrictions = Ограничения
loadouts-min-limit = Мин. кол-во: { $count }
loadouts-max-limit = Макс. кол-во: { $count }
loadouts-points-limit = Очки: { $count } / { $max }
loadouts-points-restriction = Не хватает очков
