department-Prison = Космическая тюрьма
department-Prison-description = Содержите заключенных и охраняйте тюрьму.
prisoner-major = [color=crimson]Крупная победа заключенных![/color]
prisoner-major-desc =
    Более половины заключенных сбежали с тюрьмы.
    Сбежали:
prisoner-minor = [color=crimson]Малая победа заключенных![/color]
prisoner-minor-desc =
    Хотя бы один заключенный сбежал.
    Сбежали:
prisoner-dead = [color=yellow]Все заключенные умерли![/color]
prisoner-dead-desc = Неужели вы убили каждого?!
prison-no-prisoners = [color=yellow]Заключенных не нашлось![/color]
prison-no-prisoners-desc = В этот раз повезло.
prison-minor = [color=green]Малая победа тюрьмы![/color]
prison-minor-desc = Заключенные не смогли сбежать, но некоторые умерли.
prison-major = [color=green]Победа тюрьмы![/color]
prison-major-desc = Тюрьма сохранила всех заключенных. Невозможно.
